...
export class VisualScriptingPanel extends Panel {
    ...
    /**
     * Compile the visual script into executable JavaScript code
     */
    private compileScript(): void {
        this.logger.info('Compiling visual script...');
        
        // Generate executable code from node graph using topological sort
    ...
    private generateCode(): string {
        ...
        lines.push('// Generated by WebForge Visual Scripting');
        lines.push('// ' + new Date().toISOString());
        lines.push('');
        
        // Generate variable declarations for each node output
        for (const nodeId of sorted) {
            ...
            for (const port of outputPorts) {
                const varName = `${node.type}_${node.id}_${port.id}`.replace(/[^a-zA-Z0-9_]/g, '_');
                
                // Find what inputs feed into this node
                ...
                for (const inp of inputPorts) {
                    ...
                    const conn = Array.from(this.connections.values()).find(
                        c => c.targetNode === nodeId && c.targetPort === inp.id
                    );
                    if (conn) {
                        const srcNode = this.nodes.get(conn.sourceNode);
                        if (srcNode) {
                            const srcVar = `${srcNode.type}_${srcNode.id}_${conn.sourcePort}`.replace(/[^a-zA-Z0-9_]/g, '_');
                            inputValues.push(srcVar);
                        }
                    } else if (inp.defaultValue !== undefined) {
                        inputValues.push(JSON.stringify(inp.defaultValue));
                    }
                }
                
                // Generate node-type-specific code
                const nodeCode = this.generateNodeCode(node, varName, inputValues);
                if (nodeCode) {
                    lines.push(nodeCode);
                }
            }
     ...
     */
    private generateNodeCode(node: VSScriptNode, outputVar: string, inputs: string[]): string {
        switch (node.type) {
            case 'event_start':
                return `// Event: ${node.name}`;
            case 'event_update':
                return `// Update loop (deltaTime available)`;
            case 'math_add':
                return `const ${outputVar} = (${inputs[0] || '0'}) + (${inputs[1] || '0'});`;
            case 'math_subtract':
                return `const ${outputVar} = (${inputs[0] || '0'}) - (${inputs[1] || '0'});`;
            case 'math_multiply':
                return `const ${outputVar} = (${inputs[0] || '0'}) * (${inputs[1] || '0'});`;
            case 'math_divide':
                return `const ${outputVar} = (${inputs[1] || '1'}) !== 0 ? (${inputs[0] || '0'}) / (${inputs[1] || '1'}) : 0;`;
            case 'logic_if':
                return `if (${inputs[0] || 'false'}) { /* then */ } else { /* else */ }`;
            case 'logic_and':
                return `const ${outputVar} = (${inputs[0] || 'false'}) && (${inputs[1] || 'false'});`;
            case 'logic_or':
                return `const ${outputVar} = (${inputs[0] || 'false'}) || (${inputs[1] || 'false'});`;
            case 'variable_get':
                return `const ${outputVar} = ${node.properties.get('variableName') || 'undefined'};`;
            case 'variable_set':
                return `${node.properties.get('variableName') || 'temp'} = ${inputs[0] || 'undefined'};`;
            case 'debug_log':
                return `console.log(${inputs[0] || '"debug"'});`;
            default:
                return `// Node: ${node.type} (${node.name})`;
        }
    }
    
    /**
     * Topological sort of nodes based on data flow
     */
    private topologicalSort(): string[] {
        ...
        for (const conn of this.connections.values()) {
            deps.get(conn.targetNode)?.add(conn.sourceNode);
        }
    ...
    loadFromJSON(data: any): void {
        ...
        if (data.connections) {
            for (const connData of data.connections) {
                // Look up nodes by matching old IDs to new nodes
                // Since addNodeOfType creates new IDs, we need to map by index
                const sourceNode = this.findNodeByOriginalId(data.nodes, connData.sourceNode);
                const targetNode = this.findNodeByOriginalId(data.nodes, connData.targetNode);
                
                if (sourceNode && targetNode) {
                    this.createConnection(
                        sourceNode.id, connData.sourcePort,
                        targetNode.id, connData.targetPort
                    );
                }
            }
        }
    ...
    /**
     * Find a node in the current graph that matches an original serialized node by index
     */
    private findNodeByOriginalId(originalNodes: any[], originalId: string): VSScriptNode | undefined {
        // Find the index of the original node
        const index = originalNodes?.findIndex((n: any) => n.id === originalId);
        if (index === undefined || index < 0) return undefined;
        
        // Get the node at the same index in current graph
        const currentNodes = Array.from(this.nodes.values());
        return index < currentNodes.length ? currentNodes[index] : undefined;
    }
